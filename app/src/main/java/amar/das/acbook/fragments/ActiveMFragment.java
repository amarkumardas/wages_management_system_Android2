...
//        });
        ...
        String noRateIds=db.getIdOfSpecificSkillAndReturnNullIfRateIsProvidedOfActiveOrInactiveMLG(getString(R.string.mestre),true);
        if(noRateIds == null) {
            Cursor advanceBalanceCursor = db.getData("SELECT SUM(" + Database.COL_13_ADVANCE + "),SUM(" + Database.COL_14_BALANCE + ") FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE " + Database.COL_8_MAINSKILL1 + "='" + getResources().getString(R.string.mestre) + "' AND " + Database.COL_12_ACTIVE + "='" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "'");
...
//        });
    ...
    public int getCountOfTotalRecordFromDb() {
        ...
        cursor = db.getData("SELECT COUNT() FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE " + Database.COL_8_MAINSKILL1 + "='" + getResources().getString(R.string.mestre) + "' AND " + Database.COL_12_ACTIVE + "='"+GlobalConstants.ACTIVE_PEOPLE.getValue()+"' AND " + Database.COL_15_LATESTDATE + " IS NOT NULL");
        cursor.moveToFirst();
