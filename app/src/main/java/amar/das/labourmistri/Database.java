...
import amar.das.labourmistri.pdfgenerator.MakePdf;
import amar.das.labourmistri.sharedpreferences.KeyValueTable;
import amar.das.labourmistri.sharedpreferences.SharedPreferencesHelper;
import amar.das.labourmistri.textfilegenerator.TextFile;
import amar.das.labourmistri.ui.history.HistoryFragment;
...
public class Database extends SQLiteOpenHelper {
    ...
    @Override
    //i is old version and i1 is new version.When we change version then this method is called
...
//            sqLiteDatabase.execSQL("ALTER TABLE " + TABLE_NAME3 + " ADD COLUMN PDFSEQUENCE INTEGER DEFAULT 0");
    ...
    public String insertDataToPersonRegisteredAndRateSkillTable(String name, String bankAccount, String ifscCode, String bankName, String aadhaarCard, String phoneNumber, String skill, String accountHolderName, String imagePath, String phone2, String location, String religion) {//if error return null
        ...
        try {
            ...
            if (newelyCreatedId != null && !newelyCreatedId.equals("-1")) {//inserting data to rate table
                ...
                cv.put(COL_32_R1, getDefaultRate(skill));
                ...
                if (!(success = (dB.insert(TABLE_NAME_RATE_SKILL, null, cv) == -1) ? false : true))
                    return null;// -1 is returned if error occurred.The insert() method in SQLite returns the ID of the newly created row, or -1 if there was an error inserting the data.
    ...
    private String getDefaultRate(String skill) {
        if(skill==null) return null;
        if (skill.equals(GlobalConstants.L_SKILL.getValue())) {
            return SharedPreferencesHelper.getString(context, SharedPreferencesHelper.Keys.LABOUR_DEFAULT_RATE.name(), null);
        } else if (skill.equals(GlobalConstants.M_SKILL.getValue())) {
            return SharedPreferencesHelper.getString(context, SharedPreferencesHelper.Keys.MISTRI_DEFAULT_RATE.name(), null);
        } else if (skill.equals(GlobalConstants.G_SKILL.getValue())) {
            return SharedPreferencesHelper.getString(context, SharedPreferencesHelper.Keys.WOMEN_DEFAULT_RATE.name(), null);
        }
        return null;
    }
    ...
    public boolean updateAllPersonDetails(String name, String bankAccount, String ifscCode, String bankName, String aadhaarCard, String phoneNumber, String skill, String fatherName, String imagePath, String acHolder, String id, String location, String religion) {
        if (skill == null || id == null || name == null) {
            return false;
        }
        boolean success = false;
        SQLiteDatabase dB = null;
    ...
    public boolean updatePersonSkillAndShiftData(String changedSkill, String id) {
        ...
        try {
            ...
            if (previousActiveOrInactiveAndSkill[1].equals(changedSkill) ||
                    (previousActiveOrInactiveAndSkill[1].equals(GlobalConstants.L_SKILL.getValue()) && changedSkill.equals(GlobalConstants.G_SKILL.getValue())) ||
                    (previousActiveOrInactiveAndSkill[1].equals(GlobalConstants.G_SKILL.getValue()) && changedSkill.equals(GlobalConstants.L_SKILL.getValue()))) {
                return true;
            }

            if (previousActiveOrInactiveAndSkill[0].equals(GlobalConstants.ACTIVE_PEOPLE.getValue())) {//active
...
//                }
            ...
            } else if (previousActiveOrInactiveAndSkill[0].equals(GlobalConstants.INACTIVE_PEOPLE.getValue())) {//inactive
//                if(previousActiveOrInactiveAndSkill[1].equals(context.getResources().getString(R.string.laber)) || previousActiveOrInactiveAndSkill[1].equals(context.getResources().getString(R.string.women_laber))){//if previous is G or L then definitely change to mestre because previous all condition is checked
...
//                }
    ...
    private boolean insertWagesOrDepositToActiveTableDirectly(SQLiteDatabase db, String systemDateTime, String skill, String id, String userGivenDate, String micPath, String remarks, int wages, int p1, int p2, int p3, int p4, String isDeposited) {
        ...
        ContentValues cv = new ContentValues();//to enter data at once it is like hash map
        if (skill.equals(GlobalConstants.L_SKILL.getValue()) || skill.equals(GlobalConstants.G_SKILL.getValue())) {//check for M OR lG
            if (id != null) {
        ...
        } else if (skill.equals(GlobalConstants.M_SKILL.getValue())) {//check for mestre
            if (id != null) {
    ...
    public boolean insertWagesOrDepositOnlyToActiveTableAndHistoryTableTransaction(String id, String systemCurrentDate24hrTime, String date, String micPath, String remarks, int wages, int p1, int p2, int p3, int p4, String isDeposited) {
        ...
        try {
            ...
            if (activeInactiveSkill[0].equals("1")) {//if person is active at 0th position and 1st position is person skill
                ...
                if (activeInactiveSkill[1].equals(GlobalConstants.L_SKILL.getValue()) || activeInactiveSkill[1].equals(GlobalConstants.G_SKILL.getValue())) {//check for M OR lG
                    if (id != null) {
                        cv.put(COL_1_ID_ALG, id);
                    }
                    if (date != null) {
                    ...
                    if (!(success = (dB.insert(TABLE1_ACTIVE_LG, null, cv) == -1) ? false : true))
                        return false;//if data not inserted return -1
                    //testing success=(dB.insert(TABLE2_IN_ACTIVE_MESTRE, null,cv) == -1)? false :true; //if data not inserted return -1
                } else if (activeInactiveSkill[1].equals(GlobalConstants.M_SKILL.getValue())) {//check for mestre
                    if (id != null) {
                        cv.put(COL_1_ID_AM, id);
                    }
                    if (date != null) {
                    ...
                    if (!(success = (dB.insert(TABLE0_ACTIVE_MESTRE, null, cv) == -1) ? false : true))
                        return false;//if data not inserted return -1
                    //testing success=(dB.insert(TABLE2_IN_ACTIVE_MESTRE, null,cv) == -1)? false :true; //if data not inserted return -1
                }
                //HISTORY TABLE
                if (!(success = insertWagesOrDepositToHistoryTable(dB, id, systemCurrentDate24hrTime, HistoryFragment.sameDayInserted)))
                    return false;//after inserting add to history table
    ...
    public boolean updateWagesOrDepositOnlyToActiveTableAndHistoryTableTransaction(String date, String newSystemTimeDate, String remarks, String micPath, int wages, int p1, int p2, int p3, int p4, String id, String previousSystemTimeDate) {
        ...
        try {
            ...
            if (activeInactiveSkill[0].equals(GlobalConstants.ACTIVE_PEOPLE.getValue())) {//if person is active at 0th position and 1st position is person skill
                ...
                if (!(success = updateWagesOrDepositToHistoryTable(dB, id, newSystemTimeDate, previousSystemTimeDate, previousWagesAndDepositArray, HistoryFragment.sameDayUpdated)))
                    return false;//default status code is HistoryActivity.sameDayUpdated
        ...
        try {
            ...
            if (updatedDataFromActiveTableCursor != null && updatedDataFromActiveTableCursor.getCount() == 1) {//if one row then only insert to history table
                ...
                if (MyUtility.getDateFromSystemDateTime(prevSystemDateTime).equals(MyUtility.getDateFromSystemDateTime(activeNewSystemDateTime))) {//initially on new date while updating previous record then that record prevSystemDateTime would be different from now SystemDateTime
                    ...
                    if (isThisRecordTodayAutomaticallyInsertedFromHistoryTable(id, prevSystemDateTime))
                        return false;//error
                    ...
                    if (!((dB.update(TABLE_HISTORY, c, Database.COL_1_ID_H + "= '" + id + "'" + " AND " + Database.COL_13_SYSTEM_DATETIME_H + "= '" + prevSystemDateTime + "'", null) != 1) ? false : true))
                        return false;
            ...
            if (updatedDataFromActiveTableCursor != null) {
                updatedDataFromActiveTableCursor.close();
            }//close cursor
        }
    }
    ...
    public Cursor getWagesDepositDataForRecyclerView(String id) {
        try {
            ...
            if (activeInactiveSkill[0].equals(GlobalConstants.ACTIVE_PEOPLE.getValue())) {//if person is active
                ...
                if (activeInactiveSkill[1].equals(GlobalConstants.L_SKILL.getValue()) || activeInactiveSkill[1].equals(GlobalConstants.G_SKILL.getValue())) {//check for l OR G
...
//                .append("\n").append(otherTableDataCursor.getCount()).append(" DELETED ENTRIES ARE: ");
    ...
    private Cursor getDataFromOtherTable(byte tableNumber, String id) {
        ...
        try {
            switch (tableNumber) {
                case 0:
                    return getData("SELECT " + Database.COL_2_DATE_AM + "," + Database.COL_5_REMARKS_AM + "," + Database.COL_6_WAGES_AM + "," + Database.COL_8_P1_AM + "," + Database.COL_9_P2_AM + "," + Database.COL_10_P3_AM + "," + Database.COL_11_P4_AM + "," + Database.COL_12_ISDEPOSITED_AM + " FROM " + getTableName(tableNumber) + " WHERE " + Database.COL_1_ID_AM + "='" + id + "'");
                case 1:
                    return getData("SELECT " + Database.COL_2_DATE_ALG + "," + Database.COL_5_REMARKS_ALG + "," + Database.COL_6_WAGES_ALG + "," + Database.COL_8_P1_ALG + "," + Database.COL_9_P2_ALG + "," + Database.COL_10_P3_ALG + "," + Database.COL_11_P4_ALG + "," + Database.COL_12_ISDEPOSITED_ALG + " FROM " + getTableName(tableNumber) + " WHERE " + Database.COL_1_ID_ALG + "='" + id + "'");
                case 2:
                    return getData("SELECT " + Database.COL_2_DATE_IAM + "," + Database.COL_5_REMARKS_IAM + "," + Database.COL_6_WAGES_IAM + "," + Database.COL_8_P1_IAM + "," + Database.COL_9_P2_IAM + "," + Database.COL_10_P3_IAM + "," + Database.COL_11_P4_IAM + "," + Database.COL_12_ISDEPOSITED_IAM + " FROM " + getTableName(tableNumber) + " WHERE " + Database.COL_1_ID_IAM + "='" + id + "'");
                case 3:
                    return getData("SELECT " + Database.COL_2_DATE_IALG + "," + Database.COL_5_REMARKS_IALG + "," + Database.COL_6_WAGES_IALG + "," + Database.COL_8_P1_IALG + "," + Database.COL_9_P2_IALG + "," + Database.COL_10_P3_IALG + "," + Database.COL_11_P4_IALG + "," + Database.COL_12_ISDEPOSITED_IALG + " FROM " + getTableName(tableNumber) + " WHERE " + Database.COL_1_ID_IALG + "='" + id + "'");
            }
        } catch (Exception x) {
            x.printStackTrace();
            return null;
...
//                dB.execSQL("UPDATE " + Database.PERSON_REGISTERED_TABLE + " SET " + Database.COL_12_ACTIVE + "='" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "' , "+ Database.COL_15_LATESTDATE + "='" + MyUtility.getTodaySystemDateTime24hr() + "' , " + Database.COL_16_TIME + "='" + MyUtility.getOnly12hrsTime() + "' WHERE " + Database.COL_1_ID + "='" + id + "'");//Making it active to shift data in active table
    ...
    public String getColumnName(byte tableNumberStartsWith0, byte columnIndex) {
        switch (tableNumberStartsWith0) {//hardcoded
            case 0:
                return columnNameOfTableActiveMestre(columnIndex);
            case 1:
                return columnNameOfTableActiveLaberAndG(columnIndex);
            case 2:
                return columnNameOfTableInActiveMestre(columnIndex);
            case 3:
                return columnNameOfTableInActiveLaberAndG(columnIndex);
        }
        return null;
    }
    ...
    private String columnNameOfTableActiveMestre(byte columnIndex) {
        switch (columnIndex) {
            case 1:
                return Database.COL_1_ID_AM;
            case 2:
                return Database.COL_2_DATE_AM;
            case 3:
                return Database.COL_4_MICPATH_AM;
            case 4:
                return Database.COL_5_REMARKS_AM;
            case 5:
                return Database.COL_6_WAGES_AM;
            case 6:
                return Database.COL_8_P1_AM;
            case 7:
                return Database.COL_9_P2_AM;
            case 8:
                return Database.COL_10_P3_AM;
            case 9:
                return Database.COL_11_P4_AM;
            case 10:
                return Database.COL_12_ISDEPOSITED_AM;
            case 11:
                return Database.COL_13_SYSTEM_DATETIME_AM;
        }
        return null;
    }
    ...
    private String columnNameOfTableActiveLaberAndG(byte columnIndex) {
        switch (columnIndex) {
            case 1:
                return Database.COL_1_ID_ALG;
            case 2:
                return Database.COL_2_DATE_ALG;
            case 3:
                return Database.COL_4_MICPATH_ALG;
            case 4:
                return Database.COL_5_REMARKS_ALG;
            case 5:
                return Database.COL_6_WAGES_ALG;
            case 6:
                return Database.COL_8_P1_ALG;
            case 7:
                return Database.COL_9_P2_ALG;
            case 8:
                return Database.COL_10_P3_ALG;
            case 9:
                return Database.COL_11_P4_ALG;
            case 10:
                return Database.COL_12_ISDEPOSITED_ALG;
            case 11:
                return Database.COL_13_SYSTEM_DATETIME_ALG;
        }
        return null;
    }
    ...
    private String columnNameOfTableInActiveMestre(byte columnIndex) {
        switch (columnIndex) {
            case 1:
                return Database.COL_1_ID_IAM;
            case 2:
                return Database.COL_2_DATE_IAM;
            case 3:
                return Database.COL_4_MICPATH_IAM;
            case 4:
                return Database.COL_5_REMARKS_IAM;
            case 5:
                return Database.COL_6_WAGES_IAM;
            case 6:
                return Database.COL_8_P1_IAM;
            case 7:
                return Database.COL_9_P2_IAM;
            case 8:
                return Database.COL_10_P3_IAM;
            case 9:
                return Database.COL_11_P4_IAM;
            case 10:
                return Database.COL_12_ISDEPOSITED_IAM;
            case 11:
                return Database.COL_13_SYSTEM_DATETIME_IAM;
        }
        return null;
    }
    ...
    private String columnNameOfTableInActiveLaberAndG(byte columnIndex) {
        switch (columnIndex) {
            case 1:
                return Database.COL_1_ID_IALG;
            case 2:
                return Database.COL_2_DATE_IALG;
            case 3:
                return Database.COL_4_MICPATH_IALG;
            case 4:
                return Database.COL_5_REMARKS_IALG;
            case 5:
                return Database.COL_6_WAGES_IALG;
            case 6:
                return Database.COL_8_P1_IALG;
            case 7:
                return Database.COL_9_P2_IALG;
            case 8:
                return Database.COL_10_P3_IALG;
            case 9:
                return Database.COL_11_P4_IALG;
            case 10:
                return Database.COL_12_ISDEPOSITED_IALG;
            case 11:
                return Database.COL_12_ISDEPOSITED_IALG;
        }
        return null;
    ...
    private String getTableName(byte tableNumberStartWith0) {
        switch (tableNumberStartWith0) {//hardcoded
            case 0:
                return Database.TABLE0_ACTIVE_MESTRE;
            case 1:
                return Database.TABLE1_ACTIVE_LG;
            case 2:
                return Database.TABLE2_IN_ACTIVE_MESTRE;
            case 3:
                return Database.TABLE3_IN_ACTIVE_LG;
        }
        return null;
    }

    private byte getTableNumber(String[] activeOrInactiveAndSkill) {
        if (activeOrInactiveAndSkill[0].equals(GlobalConstants.ACTIVE_PEOPLE.getValue())) {//active
            if (activeOrInactiveAndSkill[1].equals(GlobalConstants.M_SKILL.getValue())) {
                return 0;
            } else if (activeOrInactiveAndSkill[1].equals(GlobalConstants.L_SKILL.getValue()) || activeOrInactiveAndSkill[1].equals(GlobalConstants.G_SKILL.getValue())) {
                return 1;
            }
        } else if (activeOrInactiveAndSkill[0].equals(GlobalConstants.INACTIVE_PEOPLE.getValue())) {//inactive
            if (activeOrInactiveAndSkill[1].equals(GlobalConstants.M_SKILL.getValue())) {
                return 2;
            } else if (activeOrInactiveAndSkill[1].equals(GlobalConstants.L_SKILL.getValue()) || activeOrInactiveAndSkill[1].equals(GlobalConstants.G_SKILL.getValue())) {
                return 3;
...
//            }//toast will not work while doing background task
    ...
    private String generatePDFAndReturnFileAbsolutePath(String id) {//if error return null otherwise file path
        try {
            ...
            if (!makePdf.closeDocumentLastOperation4()) {
                return null;
            }
...
//                }
    ...
    private boolean fetchWorkDetailsCalculationAndWriteToPDF(String id, MakePdf makePdf) {
        try {
            ...
            if (errorDetection[0] == false) {
                if (!makeSummaryAndWriteToPDFBasedOnIndicator(context, indicator, id, makePdf, arrayOfTotalWagesDepositRateAccordingToIndicator))
                    return false;//summary
                if (!makePdf.writeSentenceWithoutLines(new String[]{""}, new float[]{100f}, true, (byte) 50, (byte) 50, true))
                    return false;//just for 1 space
                ...
                if (!addWorkAmountAndDepositBasedOnIndicatorAndWriteToPDF(indicator, arrayOfTotalWagesDepositRateAccordingToIndicator, makePdf, skillHeader)) {
                    return false;
                }
    ...
    private float[] getColumnWidthBasedOnIndicator(byte indicator, boolean[] errorDetection) {
        try {
            switch (indicator) {
                case 1:
                    return new float[]{10f, 12f, 5f, 73f};

                case 2:
                    return new float[]{10f, 12f, 5f, 5f, 68f};

                case 3:
                    return new float[]{10f, 12f, 5f, 5f, 5f, 63f};

                case 4:
                    return new float[]{10f, 12f, 5f, 5f, 5f, 5f, 58f};
            }
    ...
    private boolean makeSummaryAndWriteToPDFBasedOnIndicator(Context context, byte indicator, String id, MakePdf makePdf, int[] arrayOfTotalWagesDepositRateAccordingToIndicator) {
        try (Cursor cursor = getData("SELECT " + Database.COL_13_ADVANCE + " ," + Database.COL_14_BALANCE + " FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE " + Database.COL_1_ID + "= '" + id + "'")) {
            if (!makePdf.writeSentenceWithoutLines(new String[]{"SUMMARY", "", ""}, new float[]{12f, 50f, 38f}, false, (byte) 50, (byte) 50, true))
                return false;

            cursor.moveToFirst();//means only one row is returned
            if (cursor.getInt(0) != 0 && cursor.getInt(1) == 0) {

                if (!makePdf.singleCustomRow(MyUtility.headersForSummaryBasedOnIndicator(indicator, arrayOfTotalWagesDepositRateAccordingToIndicator, context.getResources().getString(R.string.advance_due)), new float[]{25f, 50f, 25f}, 0, 0, 0, 0, true, (byte) 50, (byte) 50))
                    return false;
                //yellow                               green
                if (!makePdf.singleCustomRow(MyUtility.totalWagesWorkAmountDepositAdvanceOrBalanceForSummaryBasedOnIndicator(indicator, arrayOfTotalWagesDepositRateAccordingToIndicator, cursor.getInt(0)), new float[]{25f, 50f, 25f}, Color.rgb(221, 133, 3), Color.rgb(26, 145, 12), Color.RED, 0, true, (byte) 50, (byte) 50))
                    return false;
                if (!makePdf.singleCustomRow(new String[]{context.getResources().getString(R.string.star_after_calculation_advance_rs_dot) + MyUtility.convertToIndianNumberSystem(cursor.getInt(0))}, new float[]{100f}, 0, 0, 0, 0, true, (byte) 50, (byte) 50))
                    return false;

            } else if (cursor.getInt(0) == 0 && cursor.getInt(1) != 0) {

                if (!makePdf.singleCustomRow(MyUtility.headersForSummaryBasedOnIndicator(indicator, arrayOfTotalWagesDepositRateAccordingToIndicator, context.getResources().getString(R.string.balance)), new float[]{25f, 50f, 25f}, 0, 0, 0, 0, true, (byte) 50, (byte) 50))
                    return false;
                //                                                                                                                                                                                                                      yellow                               green                                    green
                if (!makePdf.singleCustomRow(MyUtility.totalWagesWorkAmountDepositAdvanceOrBalanceForSummaryBasedOnIndicator(indicator, arrayOfTotalWagesDepositRateAccordingToIndicator, cursor.getInt(1)), new float[]{25f, 50f, 25f}, Color.rgb(221, 133, 3), Color.rgb(26, 145, 12), Color.rgb(26, 145, 12), 0, true, (byte) 50, (byte) 50))
                    return false;
                if (!makePdf.singleCustomRow(new String[]{context.getResources().getString(R.string.star_after_calculation_balance_rs_dot) + MyUtility.convertToIndianNumberSystem(cursor.getInt(1))}, new float[]{100f}, 0, 0, 0, 0, true, (byte) 50, (byte) 50))
                    return false;

            } else if (cursor.getInt(0) == 0 && cursor.getInt(1) == 0) {
                if (!makePdf.singleCustomRow(MyUtility.headersForSummaryBasedOnIndicator(indicator, arrayOfTotalWagesDepositRateAccordingToIndicator, context.getResources().getString(R.string.all_cleared)), new float[]{25f, 50f, 25f}, 0, 0, 0, 0, true, (byte) 50, (byte) 50))
                    return false;
                // yellow                               green                                green
                if (!makePdf.singleCustomRow(MyUtility.totalWagesWorkAmountDepositAdvanceOrBalanceForSummaryBasedOnIndicator(indicator, arrayOfTotalWagesDepositRateAccordingToIndicator, 0), new float[]{25f, 50f, 25f}, Color.rgb(221, 133, 3), Color.rgb(26, 145, 12), Color.rgb(26, 145, 12), 0, true, (byte) 50, (byte) 50))
                    return false;
                if (!makePdf.singleCustomRow(new String[]{context.getResources().getString(R.string.star_after_calculation_no_dues_dot)}, new float[]{100f}, 0, 0, 0, 0, true, (byte) 50, (byte) 50))
                    return false;
...
//            cv.put(COL_6_WAGES_H,cursor.getString(3));
    ...
    public String getMicPath(String id, String systemDateTime) {//String id,String systemDateTime this two variables act as primary key
        try (Cursor cursor = getData("SELECT " + getColumnNameOutOf4Table(id, (byte) 3) + " FROM " + tableNameOutOf4Table(id) + " WHERE " + getColumnNameOutOf4Table(id, (byte) 1) + "= '" + id + "'" + " AND " + getColumnNameOutOf4Table(id, (byte) 11) + "= '" + systemDateTime + "'")) {
            if (cursor != null && cursor.getCount() == 0)
                return null;//if no record found based on primary key then it will give error cursorIndexoutofBound exception but this will not happen.So checking cursor size if size is 0 return null otherwise this cursor.moveToFirst(); will give error as cursorIndexoutofBound exception
            cursor.moveToFirst();
            return cursor.getString(0);
        } catch (Exception x) {
            x.printStackTrace();
    ...
    public HashMap<Character, Integer> getTotalPeopleWorked(int year, byte month, byte day, String skillMLGColumnName, String p1p2p3p4workDaysColumnName) {//return format M 6 L 0 G 3. if error return null
        ...
        StringBuilder query = new StringBuilder(300)
                ...
                .append(GlobalConstants.M_SKILL.getValue()).append("' AS skill UNION SELECT '")
                .append(GlobalConstants.L_SKILL.getValue()).append("' UNION SELECT '")
                .append(GlobalConstants.G_SKILL.getValue()).append("') AS people LEFT JOIN ")
                ...
                .append(" AND ").append(Database.TABLE_HISTORY).append("." + Database.COL_19_STATUS_H + " IN (" + HistoryFragment.sameDayInserted + ", " + HistoryFragment.sameDayUpdated + ")")  // if status code is 1 or 2 that means to worked person only
...
//            }
            ...
            cursor = getData("SELECT " + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE " + Database.COL_8_MAINSKILL1 + "='" + GlobalConstants.M_SKILL.getValue() + "' AND " + Database.COL_12_ACTIVE + "='" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "'");//no need to check for latest date is null or not we need only active ids
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    idList.add(cursor.getInt(0));
                }
...
//            }
            ...
            cursor = getData("SELECT " + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE (" + Database.COL_8_MAINSKILL1 + "='" + GlobalConstants.L_SKILL.getValue() + "' OR " + Database.COL_8_MAINSKILL1 + "='" + GlobalConstants.G_SKILL.getValue() + "') AND " + Database.COL_12_ACTIVE + "='" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "'");
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    idList.add(cursor.getInt(0));
                }
    ...
    public String[] getIdOfInActiveMOrLOrG(String skillType) {//if error return null
        try {
            List<Integer> idList = new ArrayList<>();//insertion is fast
            Cursor cursor = null;
            if (skillType.equals(GlobalConstants.M_SKILL.getValue())) {
...
//                }
                ...
                cursor = getData("SELECT " + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE " + Database.COL_8_MAINSKILL1 + "='" + GlobalConstants.M_SKILL.getValue() + "' AND " + Database.COL_12_ACTIVE + "='" + GlobalConstants.INACTIVE_PEOPLE.getValue() + "'");//no need to check for latest date we need only inactive data
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        idList.add(cursor.getInt(0));
                    }
            ...
            } else if (skillType.equals(GlobalConstants.L_SKILL.getValue())) {
...
//                }
                ...
                cursor = getData("SELECT " + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE " + Database.COL_8_MAINSKILL1 + "='" + GlobalConstants.L_SKILL.getValue() + "' AND " + Database.COL_12_ACTIVE + "='" + GlobalConstants.INACTIVE_PEOPLE.getValue() + "'");
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        idList.add(cursor.getInt(0));
                    }
            ...
            } else if (skillType.equals(GlobalConstants.G_SKILL.getValue())) {
...
//                }
                ...
                cursor = getData("SELECT " + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " WHERE " + Database.COL_8_MAINSKILL1 + "='" + GlobalConstants.G_SKILL.getValue() + "' AND " + Database.COL_12_ACTIVE + "='" + GlobalConstants.INACTIVE_PEOPLE.getValue() + "'");
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        idList.add(cursor.getInt(0));
                    }
...
//        AND( r.INDICATOR = 1 OR r.INDICATOR IS NULL);
    ...
    public String getIdOfLGSkillAndReturnNullIfRateIsProvidedOfActive() {
        ...
        try {//getting data from two table
            StringBuilder resultBuilder = new StringBuilder();
            cursor = getData("SELECT pd." + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " pd INNER JOIN " + Database.TABLE_NAME_RATE_SKILL + " r ON pd." + Database.COL_1_ID + " = r." + Database.COL_31_ID + " WHERE pd." + Database.COL_12_ACTIVE + " = '" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "' AND (pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.L_SKILL.getValue() + "' OR pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.G_SKILL.getValue() + "') AND( r." + Database.COL_39_INDICATOR + " = '" + 1 + "' OR r." + Database.COL_39_INDICATOR + " IS NULL) AND r." + Database.COL_32_R1 + " IS NULL");//initially when new person is created then indicator is set to null when again second skill is added and removed second skill that time indicator is set to 1 so checking 1 or null for indicator
            if (cursor != null && cursor.getCount() != 0) {//if no record found based on primary key then it will give error cursorIndexoutofBound exception but this will not happen.So checking cursor size if size is 0 return null otherwise this cursor.moveToFirst(); will give error as cursorIndexoutofBound exception
                while (cursor.moveToNext()) {
                    resultBuilder.append(cursor.getString(0)).append(" , ");
                }
            ...
            cursor = getData("SELECT pd." + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " pd INNER JOIN " + Database.TABLE_NAME_RATE_SKILL + " r ON pd." + Database.COL_1_ID + " = r." + Database.COL_31_ID + " WHERE pd." + Database.COL_12_ACTIVE + " = '" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "' AND (pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.L_SKILL.getValue() + "' OR pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.G_SKILL.getValue() + "') AND r." + Database.COL_39_INDICATOR + " = '" + 2 + "' AND (r." + Database.COL_32_R1 + " IS NULL" + " OR  r." + Database.COL_33_R2 + " IS NULL)");
            if (cursor != null && cursor.getCount() != 0) {//if no record found based on primary key then it will give error cursorIndexoutofBound exception but this will not happen.So checking cursor size if size is 0 return null otherwise this cursor.moveToFirst(); will give error as cursorIndexoutofBound exception
                while (cursor.moveToNext()) {
                    resultBuilder.append(cursor.getString(0)).append(" , ");
                }
            ...
            cursor = getData("SELECT pd." + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " pd INNER JOIN " + Database.TABLE_NAME_RATE_SKILL + " r ON pd." + Database.COL_1_ID + " = r." + Database.COL_31_ID + " WHERE pd." + Database.COL_12_ACTIVE + " = '" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "' AND (pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.L_SKILL.getValue() + "' OR pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.G_SKILL.getValue() + "') AND r." + Database.COL_39_INDICATOR + " = '" + 3 + "' AND (r." + Database.COL_32_R1 + " IS NULL" + " OR  r." + Database.COL_33_R2 + " IS NULL" + " OR  r." + Database.COL_34_R3 + " IS NULL)");
            if (cursor != null && cursor.getCount() != 0) {//if no record found based on primary key then it will give error cursorIndexoutofBound exception but this will not happen.So checking cursor size if size is 0 return null otherwise this cursor.moveToFirst(); will give error as cursorIndexoutofBound exception
                while (cursor.moveToNext()) {
                    resultBuilder.append(cursor.getString(0)).append(" , ");
                }
            ...
            cursor = getData("SELECT pd." + Database.COL_1_ID + " FROM " + Database.PERSON_REGISTERED_TABLE + " pd INNER JOIN " + Database.TABLE_NAME_RATE_SKILL + " r ON pd." + Database.COL_1_ID + " = r." + Database.COL_31_ID + " WHERE pd." + Database.COL_12_ACTIVE + " = '" + GlobalConstants.ACTIVE_PEOPLE.getValue() + "' AND (pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.L_SKILL.getValue() + "' OR pd." + Database.COL_8_MAINSKILL1 + "= '" + GlobalConstants.G_SKILL.getValue() + "') AND r." + Database.COL_39_INDICATOR + " = '" + 4 + "' AND (r." + Database.COL_32_R1 + " IS NULL" + " OR  r." + Database.COL_33_R2 + " IS NULL" + " OR  r." + Database.COL_34_R3 + " IS NULL OR r." + Database.COL_35_R4 + " IS NULL)");
            if (cursor != null && cursor.getCount() != 0) {//if no record found based on primary key then it will give error cursorIndexoutofBound exception but this will not happen.So checking cursor size if size is 0 return null otherwise this cursor.moveToFirst(); will give error as cursorIndexoutofBound exception
                while (cursor.moveToNext()) {
                    resultBuilder.append(cursor.getString(0)).append(" , ");
                }
    ...
    public File databaseBackup(String uniqueDatabaseFileName) {//duplicating the db
        if (!updateTotalNumberOfRowsToKeyValueTable())
            return null;//before data backup update total no. of active and inactive rows to column so that we can check this value when restore database is fully restore or not

        File databaseFile = context.getDatabasePath(DATABASE_NAME);

        if (!databaseFile.exists()) {
            return null;
        } // Check if database exists and return null if not
...
//            return backupFile;
    ...
    private File createDbFolderInExternalStorageAndReturnFile(String uniqueFileName, String fileExtension) {//return null when exception
        try {//externalFileDir is passed as string because this class is not extended with AppCompatActivity
            ...
            if (!MyUtility.isFolderExistIfNotExistCreateIt(context.getExternalFilesDir(null).toString(), GlobalConstants.DATABASE_FOLDER_NAME.getValue()))
                return null;
    ...
    public boolean setDefaultRateShouldBeCalledAfterUpdatingIndicator(String id, byte givenIndicator, Context context, String skill) {
        if (MyUtility.get_indicator(context, id) != givenIndicator){//if givenIndicator and personIndicator is not same than we cannot update otherwise we will update rate which would be useless
            return false;
        }
        switch (givenIndicator) {
            case 1: {
                return updateSingleRate(id,skill,Database.COL_32_R1);
            }
            case 2: {
                return  updateSingleRate(id,skill,Database.COL_33_R2);
            }
            case 3: {
                return  updateSingleRate(id,skill,Database.COL_34_R3);
            }
            case 4: {
                return updateSingleRate(id, skill,Database.COL_35_R4);
            }
        }
        return false;
    }

    public boolean updateSingleRate(String id, String skill, String columnName) {
        if(id==null || skill==null || columnName==null) return false;//its important else if skill is null than getDefaultRate(skill) method will return null but in database null string would be stored

      return updateTable("UPDATE " + Database.TABLE_NAME_RATE_SKILL + " SET '" + columnName + "'=" +((getDefaultRate(skill) != null)? ("'"+getDefaultRate(skill)+"'"):"NULL") + " WHERE " + Database.COL_31_ID + "= '" + id + "'");
    }
}
