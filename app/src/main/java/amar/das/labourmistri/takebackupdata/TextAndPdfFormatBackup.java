...
import amar.das.labourmistri.activity.PdfViewerOperationActivity;
...
public class TextAndPdfFormatBackup {
    ...
    private StringBuilder createdInfoAndTotalAdvanceAndBalanceForSingleBackupTextFile(int numberOfPerson,byte fileIndicator){
        ...
        switch (fileIndicator) {
            case 0:{
                array=BackupDataUtility.getActiveSkillMLGCreatedInfo(numberOfPerson);
            ...
            case 1: {
                array=BackupDataUtility.getInActiveSkillCreatedInfo(numberOfPerson, GlobalConstants.M_SKILL.getValue());
                sb.append(array[0]).append("\n").append(array[1]).append("\n\n");
                sb.append(BackupDataUtility.getTotalInActiveMOrLOrGAdvanceAndBalanceInfo(context,GlobalConstants.M_SKILL.getValue())[0]).append("\n-----------------------------\n\n");
                return sb;
            }
            case 2: {
                array=BackupDataUtility.getInActiveSkillCreatedInfo(numberOfPerson,GlobalConstants.L_SKILL.getValue());
                sb.append(array[0]).append("\n").append(array[1]).append("\n\n");
                sb.append(BackupDataUtility.getTotalInActiveMOrLOrGAdvanceAndBalanceInfo(context,GlobalConstants.L_SKILL.getValue())[0]).append("\n-----------------------------\n\n");
                return sb;
            }
            case 3: {
                array=BackupDataUtility.getInActiveSkillCreatedInfo(numberOfPerson,GlobalConstants.G_SKILL.getValue());
                sb.append(array[0]).append("\n").append(array[1]).append("\n\n");
                sb.append(BackupDataUtility.getTotalInActiveMOrLOrGAdvanceAndBalanceInfo(context,GlobalConstants.G_SKILL.getValue())[0]).append("\n-----------------------------\n\n");
                return sb;
            }
            default:return null;
        }
    ...
    private String[] createdInfoAndTotalAdvanceAndBalanceForSingleBackupPdfFile(MakePdf makePdf, byte fileIndicator) {//if error return null
        ...
        if(fileIndicator==0){//for active mlg
            ...
            createInfo=BackupDataUtility.getActiveSkillMLGCreatedInfo(personIds.length);
            ...
            personIds=db.getIdOfInActiveMOrLOrG(GlobalConstants.M_SKILL.getValue()); if(personIds==null) return  null;//if error
            createInfo=BackupDataUtility.getInActiveSkillCreatedInfo(personIds.length,GlobalConstants.M_SKILL.getValue());
            totalAdvanceAndBalance=BackupDataUtility.getTotalInActiveMOrLOrGAdvanceAndBalanceInfo(context,GlobalConstants.M_SKILL.getValue());
        }else if (fileIndicator==2) {//for inactive skill l
            personIds=db.getIdOfInActiveMOrLOrG(GlobalConstants.L_SKILL.getValue()); if(personIds==null) return  null;//if error
            createInfo=BackupDataUtility.getInActiveSkillCreatedInfo(personIds.length,GlobalConstants.L_SKILL.getValue());
            totalAdvanceAndBalance=BackupDataUtility.getTotalInActiveMOrLOrGAdvanceAndBalanceInfo(context,GlobalConstants.L_SKILL.getValue());
        }else if (fileIndicator==3) {//for inactive skill g
            personIds=db.getIdOfInActiveMOrLOrG(GlobalConstants.G_SKILL.getValue()); if(personIds==null) return  null;//if error
            createInfo=BackupDataUtility.getInActiveSkillCreatedInfo(personIds.length,GlobalConstants.G_SKILL.getValue());
            totalAdvanceAndBalance=BackupDataUtility.getTotalInActiveMOrLOrGAdvanceAndBalanceInfo(context,GlobalConstants.G_SKILL.getValue());
        }
    ...
    public File backupActiveMLGDataInPDFFormat(String backupPdfFileName){//if error return null
        ...
        if(!makePdf.singleCustomRow(BackupDataUtility.getActiveSkillMLGCreatedInfo(personIds.length),new float[]{30f,70f},Color.rgb(10, 178, 21),Color.rgb(10, 178, 21), 0, 0, true, (byte) 0, (byte) 0)) return null;
    ...
    public String backupActiveMLGDataInTextFormat(){//if error return null
        ...
        StringBuilder sb=new StringBuilder();
        String[] array=BackupDataUtility.getActiveSkillMLGCreatedInfo(personIds.length);
        sb.append(array[0]).append("\n").append(array[1]).append("\n\n");
